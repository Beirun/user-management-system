    ...
    <main class="container py-8">
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-4">
          ...
          <div v-else-if="displayEmployees.length === 0"
            ...
            <p class="text-sm text-muted-foreground">Try adjusting your search or add a new employee record.</p>
          </div>
          <!-- Employee Card Grid -->
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <div v-for="employee in displayEmployees" :key="employee.id"
              ...
              <div class="flex items-start gap-4 mb-4">
                ...
                <div class="flex-1 space-y-0.5">
                  <h3 class="font-semibold text-lg leading-tight"> {{ employee.accountDetails?.title }} {{
                    employee.accountDetails?.firstName }} {{ employee.accountDetails?.lastName }} </h3>
                  <p class="text-sm text-primary">{{ employee.position }}</p>
                  <p class="text-xs text-muted-foreground">ID: {{ employee.employeeIdString }}</p>
                </div>
                <DropdownMenu>
                  <DropdownMenuTrigger as-child> <Button variant="ghost"
                      class="h-8 w-8 p-0 self-start text-muted-foreground hover:text-foreground">
                      <MoreHorizontal class="h-5 w-5" />
                    </Button> </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" class="w-52">
                    <DropdownMenuItem @click="openEditEmployeeDialog(employees.find(e => e.id === employee.id)!)">
                      <User class="mr-2 h-4 w-4" /> <span>Edit Record</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem @click="openTransferDialog(employees.find(e => e.id === employee.id)!)">
                      <Briefcase class="mr-2 h-4 w-4" /> <span>Transfer Department</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem @click="openWorkflowsDialog(employees.find(e => e.id === employee.id)!)">
                      <RefreshCw class="mr-2 h-4 w-4" /> <span>View Workflows</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem @click="openEmployeeRequestsDialog(employees.find(e => e.id === employee.id)!)">
                      <FileText class="mr-2 h-4 w-4" /> <span>View Requests</span>
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
              <div class="space-y-2.5 text-sm mb-4 flex-grow">
                <div v-if="employee.accountDetails" class="flex items-center">
                  <User class="mr-2 h-4 w-4 text-muted-foreground flex-shrink-0" /> <a
                    :href="'mailto:' + employee.accountDetails.email" class="truncate text-primary hover:underline"
                    :title="employee.accountDetails.email"> {{ employee.accountDetails.email }} </a>
                </div>
                <div v-if="employee.departmentDetails" class="flex items-center">
                  <Briefcase class="mr-2 h-4 w-4 text-muted-foreground flex-shrink-0" /> <span class="truncate"
                    :title="employee.departmentDetails.name">Dept: {{ employee.departmentDetails.name }}</span>
                </div>
                <div class="flex items-center">
                  <CalendarIcon class="mr-2 h-4 w-4 text-muted-foreground flex-shrink-0" /> <span>Hired: {{
                    formatDisplayDate(employee.hireDate) }}</span>
                </div>
              </div>
    ...
    <Dialog v-model:open="isEmployeeEditDialogOpen">
      <DialogContent class="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>Update the employee's assignment and status.</DialogDescription>
        </DialogHeader>
          ...
          <div class="space-y-8 max-h-[60vh] overflow-y-auto pr-3 -mr-3">
            <div class="grid grid-cols-5 gap-4">
              <div class="space-y-1.5 col-span-2"> <Label>Employee ID (System)</Label>
                <p
                  class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-muted-foreground min-h-[2.5rem] flex items-center">
              ...
              <div class="space-y-1.5 col-span-3"> <Label for="edit-account">Account *</Label>
                <Select id="edit-account" v-model="selectedEmployee.accountId" required>
                  ...
                  <SelectContent>
                    <SelectItem
                      v-for="acc in accounts.filter(acc => !employees.some(e => e.accountId === Number(acc.id) && e.id !== selectedEmployee?.id) || selectedEmployee?.accountId === Number(acc.id))"
            ...
            <div class="grid grid-cols-5 gap-4">
              ...
              <div class="space-y-1.5 col-span-3"> <Label for="edit-position">Position *</Label> <Input
                  id="edit-position" v-model="selectedEmployee.position" placeholder="Software Engineer" required
                  class="w-full" /> </div>
            </div>
          </div>
          <DialogFooter class="flex justify-end gap-2 pt-6 mt-8 border-t"> <Button variant="outline"
              @click="isEmployeeEditDialogOpen = false">Cancel</Button> <Button :disabled="isEditingEmployee"
              class="text-foreground" @click="saveEditedEmployee">
              <RefreshCw v-if="isEditingEmployee" class="mr-2 h-4 w-4 animate-spin" />
    ...
    <Dialog v-model:open="isAddEmployeeDialogOpen">
      <DialogContent class="sm:max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>Assign an account to an employee role.</DialogDescription>
        </DialogHeader>
        <div class="py-4">
          <div class="space-y-8 max-h-[70vh] overflow-y-auto pr-3 -mr-3">
            <div class="grid grid-cols-1 gap-4">
              <div class="space-y-1.5"> <Label for="new-account">Account *</Label>
            ...
            <div class="grid grid-cols-5 gap-4">
              ...
              <div class="space-y-1.5 col-span-3"> <Label for="new-position">Position *</Label> <Input id="new-position"
                  v-model="newEmployeeForm.position" placeholder="E.g., Software Developer" required class="w-full" />
              </div>
            </div>
            <div class="grid grid-cols-5 gap-4">
              <div class="space-y-1.5 col-span-2"> <Label for="new-hireDate">Hire Date *</Label>
                <Popover>
                  <PopoverTrigger as-child> <Button variant="outline"
                      :class="cn('w-full justify-start text-left font-normal', !newHireDateForPicker && 'text-muted-foreground')">
                      <CalendarIcon class="mr-2 h-4 w-4" /> {{ newHireDateForPicker ?
                        dateFormatter.format(newHireDateForPicker.toDate(getLocalTimeZone())) : "Pick a date" }}
                    </Button> </PopoverTrigger>
                  <PopoverContent class="w-auto p-0">
                    <Calendar v-model="newHireDateForPicker" initial-focus :min-value="parseDateValue('1950-01-01')"
          ...
          <DialogFooter class="flex justify-end gap-2 pt-6 mt-8 border-t"> <Button variant="outline"
              @click="isAddEmployeeDialogOpen = false">Cancel</Button> <Button :disabled="isAddingEmployee"
              class="text-foreground" @click="addNewEmployeeHandler">
              <RefreshCw v-if="isAddingEmployee" class="mr-2 h-4 w-4 animate-spin" />
      ...
      <DialogContent class="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription v-if="employeeToTransfer && getDisplayEmployee(employeeToTransfer)?.accountDetails">
            Select a new department for {{ getDisplayEmployee(employeeToTransfer)!.accountDetails!.firstName }} {{
              getDisplayEmployee(employeeToTransfer)!.accountDetails!.lastName }} (ID: {{
              formatEmployeeId(employeeToTransfer.id) }}).
          </DialogDescription>
        ...
        <div v-if="employeeToTransfer" class="py-4">
          <div class="space-y-6 max-h-[60vh] overflow-y-auto pr-3 -mr-3">
            <div class="space-y-2"> <Label>Current Department</Label>
              <p
                class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-muted-foreground min-h-[2.5rem] flex items-center">
                {{ selectedCurrentDepartmentName }} </p>
            </div>
            <div class="space-y-2"> <Label for="new-dept-transfer">New Department *</Label>
              <Select id="new-dept-transfer" v-model="newDepartmentForTransfer" required>
                ...
                <SelectContent>
                  <SelectItem v-for="dept in departments" :key="dept.id" :value="dept"
                    :disabled="dept.id === employeeToTransfer.departmentId"> {{ dept.name }} </SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          <DialogFooter class="flex justify-end gap-2 pt-6 mt-4 border-t"> <Button variant="outline"
              @click="isTransferDialogOpen = false">Cancel</Button> <Button :disabled="isTransferringEmployee"
              @click="handleTransfer" class="text-foreground">
              <RefreshCw v-if="isTransferringEmployee" class="mr-2 h-4 w-4 animate-spin" />
              {{ isTransferringEmployee ? 'Transferring Employee...' : 'Transfer Employee' }}
            </Button> </DialogFooter>
        </div>
      </DialogContent>
    </Dialog>
    ...
    <Dialog v-model:open="isWorkflowsDialogOpen">
      <DialogContent class="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription v-if="employeeForWorkflows && getDisplayEmployee(employeeForWorkflows)?.accountDetails">
            Workflows for {{ getDisplayEmployee(employeeForWorkflows)!.accountDetails!.firstName }} {{
              getDisplayEmployee(employeeForWorkflows)!.accountDetails!.lastName }} (ID: {{
              formatEmployeeId(employeeForWorkflows.id) }}). </DialogDescription>
          <DialogDescription v-else-if="employeeForWorkflows"> Workflows for Employee ID: {{
            formatEmployeeId(employeeForWorkflows.id) }}. </DialogDescription>
        </DialogHeader>
        <div class="py-4 max-h-[70vh] overflow-y-auto">
          <div v-if="!workflowsForSelectedEmployee || workflowsForSelectedEmployee.length === 0"
            class="text-center py-10">
            <p class="text-muted-foreground">No workflows found for this employee.</p>
          </div>
          <div v-else class="space-y-3">
            <div v-for="wf in workflowsForSelectedEmployee" :key="wf.id" class="border rounded-md p-3">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-2 items-start">
                <div>
                  <p class="font-medium">{{ wf.type }}</p>
                  <p class="text-xs text-muted-foreground">Submitted: {{ formatDisplayDate(wf.created.slice(0, 10)) }}
                  </p>
                </div>
                <div class="md:col-span-2">
                  <p class="text-sm">{{ wf.details }}</p>
                </div>
              </div>
              <div
                class="mt-2 pt-2 border-t flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <Badge :class="cn('capitalize', getRequestCardStatusBadgeClass(wf.status))"> {{ wf.status }} </Badge>
                <DropdownMenu>
                  <DropdownMenuTrigger as-child> <Button :disabled="isUpdatingWorkflow" variant="outline" size="sm">
                      <RefreshCw v-if="isUpdatingWorkflow" class="mr-2 h-4 w-4 animate-spin" />
                      {{ isUpdatingWorkflow ? 'Updating...' : 'Update Status' }}
                      <MoreHorizontal class="ml-1 h-4 w-4" />
                    </Button> </DropdownMenuTrigger>
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem v-for="statusOpt in workflowStatusOptionsFromApi.filter(s => s !== wf.status)"
                      :key="statusOpt" @click="updateWorkflowStatus(employeeForWorkflows!.id, wf.id, statusOpt)"> Set to
                      {{ statusOpt }} </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            </div>
        ...
        <DialogFooter class="pt-4 mt-4 border-t"> <Button variant="outline"
            @click="isWorkflowsDialogOpen = false">Close</Button> </DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <Dialog v-model:open="isEmployeeRequestsDialogOpen">
      <DialogContent class="sm:max-w-5xl">
        <DialogHeader>
          ...
          <DialogDescription v-if="employeeForRequests && getDisplayEmployee(employeeForRequests)?.accountDetails">
            Requests for {{ getDisplayEmployee(employeeForRequests)!.accountDetails!.firstName }} {{
              getDisplayEmployee(employeeForRequests)!.accountDetails!.lastName }} (ID: {{
              formatEmployeeId(employeeForRequests.id) }}). </DialogDescription>
          <DialogDescription v-else-if="employeeForRequests"> Requests for Employee ID: {{
            formatEmployeeId(employeeForRequests.id) }}. </DialogDescription>
        </DialogHeader>
        <div class="py-4 max-h-[70vh] overflow-y-auto pr-2 -mr-2">
          <div v-if="!requestsForSelectedEmployee || requestsForSelectedEmployee.length === 0"
            class="text-center py-10">
            <p class="text-muted-foreground">No requests found for this employee.</p>
          </div>
          <div v-else class="grid grid-cols-2 md:grid-cols-3 gap-4">
            <Card v-for="req_basic in requestsForSelectedEmployee" :key="req_basic.id"
              class="flex flex-col bg-card text-card-foreground rounded-lg p-4 border-card">
              <CardHeader>
                <div class="flex justify-between items-start gap-2">
                  <div class="pb-4">
                    <CardTitle class="text-lg flex items-center gap-2">
                      <component :is="getRequestCardIcon(req_basic.type)" class="h-5 w-5 text-primary flex-shrink-0" />
                      {{ req_basic.type }} Request
                    </CardTitle>
                    <CardDescription class="text-xs">ID: {{ req_basic.id }}</CardDescription>
                  </div>
                  <Badge
                    :class="cn('text-xs px-2 py-1 capitalize whitespace-nowrap', getRequestCardStatusBadgeClass(req_basic.status))">
                    {{ req_basic.status }} </Badge>
                </div>
              </CardHeader>
              <CardContent class="flex-grow space-y-2 text-sm">
                ...
                <template v-if="req_basic.type.toLowerCase().includes('leave') && req_basic.requestLeave">
                  <p><strong class="text-muted-foreground">Start:</strong> {{
                    formatDisplayDate(req_basic.requestLeave.startDate) }}</p>
                  <p><strong class="text-muted-foreground">End:</strong> {{
                    formatDisplayDate(req_basic.requestLeave.endDate) }}</p>
                </template>
                <template
                  v-if="(req_basic.type.toLowerCase().includes('equipment') || req_basic.type.toLowerCase().includes('resource')) && req_basic.requestItems && req_basic.requestItems.length > 0">
                  <p class="font-medium text-muted-foreground mt-1">{{ req_basic.type === 'Equipment' ? 'Items' :
                    'Resources' }}:</p>
                  <ul class="list-disc list-inside ml-1 space-y-0.5 pb-2">
                    <li v-for="(item, itemIdx) in req_basic.requestItems.slice(0, 3)"
                      :key="`dialog-req-${req_basic.id}-item-${itemIdx}`"> {{ item.name }} (Qty: {{ item.quantity }})
                    </li>
                  </ul>
                  <p v-if="req_basic.requestItems.length > 3" class="text-xs text-muted-foreground ml-5 italic">+ {{
                    req_basic.requestItems.length - 3 }} more...</p>
                </template>
                <template
                  v-if="(req_basic.type.toLowerCase().includes('equipment') || req_basic.type.toLowerCase().includes('resource')) && (!req_basic.requestItems || req_basic.requestItems.length === 0)">
                  <p class="text-sm italic text-muted-foreground mt-1">No items specified.</p>
                </template>
              </CardContent>
              <CardFooter class="border-t pt-4">
                <Button variant="outline" size="sm"
                  @click="() => { console.log(req_basic); const unifiedReq = mapBasicRequestToFrontendUnified(req_basic, employeeForRequests!.id); if (unifiedReq) { openMainRequestEditDialog(unifiedReq); } else { toastService.error({ title: 'Error', description: 'Could not prepare request for editing.' } as Toast); } }"
                  class="w-full">
        ...
        <DialogFooter class="pt-4 mt-4 border-t"> <Button variant="outline"
            @click="isEmployeeRequestsDialogOpen = false">Close</Button> </DialogFooter>
      </DialogContent>
    </Dialog>
        ...
        <div v-if="selectedRequestForMainEdit" class="grid gap-8 py-4 max-h-[70vh] overflow-y-auto -mr-4 w-full">
          <div class="grid grid-cols-2 gap-4 w-full">
            <div class="space-y-1.5 w-full">
              ...
              <p class="w-full rounded-md border border-input bg-muted/30 px-3 py-1.75 opacity-80 text-sm text-muted-foreground flex items-center"
                id="main-edit-employee">{{ `${getEmployeeFullName(selectedRequestForMainEdit.employeeId)}
                (${formatEmployeeId(selectedRequestForMainEdit.employeeId!)})` }}</p>
            </div>
              ...
              <Popover>
                <PopoverTrigger as-child>
                  <Button variant="outline"
                    ...
                    {{ mainEditRequestDateForPicker ?
                      dateFormatter.format(mainEditRequestDateForPicker.toDate(getLocalTimeZone())) : "Pick a date" }}
                  </Button>
                </PopoverTrigger>
              </Popover>
            </div>
          ...
          <div class="grid grid-cols-2 gap-4 w-full">
            ...
            <div class="space-y-1.5 w-full">
              ...
              <Select id="main-edit-request-status" v-model="selectedRequestForMainEdit.status">
                ...
                <SelectContent>
                  <SelectItem v-for="statusOpt in mainRequestStatusOptions" :key="`main-edit-status-${statusOpt}`"
                    :value="statusOpt">{{ statusOpt }}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <template
            v-if="selectedRequestForMainEdit.type === 'Equipment' || selectedRequestForMainEdit.type === 'Resources'">
            <div class="space-y-3 border p-3 rounded-md bg-muted/30">
              <Label class="font-semibold">{{ selectedRequestForMainEdit.type === 'Equipment' ? 'Equipment Items' :
                'Resources' }} *</Label>
              <div v-if="selectedRequestForMainEdit.items.length > 0" class="space-y-2">
                <div v-for="(item, index) in selectedRequestForMainEdit.items" :key="`main-edit-item-${index}`"
                  class="grid grid-cols-10 gap-2 items-center">
                  <div class="col-span-6">
                    <Input v-model="item.name"
                      :placeholder="selectedRequestForMainEdit.type === 'Equipment' ? 'Item Name' : 'Resource Name'"
                      class="text-sm h-9" />
                  </div>
                  <div class="col-span-3">
                    <Input v-model.number="item.quantity" type="number"
                      :placeholder="selectedRequestForMainEdit.type === 'Equipment' ? 'Qty' : 'Units'" min="1"
                      class="text-sm h-9" />
                  </div>
              ...
              <Button @click="addRowToMainEditForm" variant="outline" size="sm" class="mt-2 w-full">
                <PlusCircle class="mr-2 h-4 w-4" /> Add New {{ selectedRequestForMainEdit.type === 'Equipment' ? 'Item'
                  : 'Resource' }}
              </Button>
            </div>
          </template>
          ...
          <template v-else-if="selectedRequestForMainEdit.type === 'Leave'">
            <div class="space-y-1.5 w-full">
              ...
              <Popover>
                <PopoverTrigger as-child>
                  <Button variant="outline"
                    :class="cn('w-full justify-start text-left font-normal', !mainEditStartDateForPicker && 'text-muted-foreground')">
                    <CalendarDays class="mr-2 h-4 w-4" />
                    {{ mainEditStartDateForPicker ?
                      dateFormatter.format(mainEditStartDateForPicker.toDate(getLocalTimeZone())) : "Pick a start date" }}
                  </Button>
                </PopoverTrigger>
                <PopoverContent class="w-auto p-0">
            ...
            <div class="space-y-1.5 w-full">
              ...
              <Popover>
                <PopoverTrigger as-child>
                  <Button variant="outline"
                    :class="cn('w-full justify-start text-left font-normal', !mainEditEndDateForPicker && 'text-muted-foreground')">
                    <CalendarDays class="mr-2 h-4 w-4" />
                    {{ mainEditEndDateForPicker ?
                      dateFormatter.format(mainEditEndDateForPicker.toDate(getLocalTimeZone())) : "Pick an end date" }}
                  </Button>
                </PopoverTrigger>
                <PopoverContent class="w-auto p-0">
